...
const VendorGeneratePDF = (dataa, pData) => {
  console.log(pData);
  const unit = 'px';
  const size = 'A4';
  ...
  doc.setFontSize(10);
  const reportText=`${pData.report} REPORT`
  const uptext=reportText.toUpperCase()
  doc.text(uptext, margin, 145);
  ...
  for (let i = 0; i < dataa.length; i++) {
    ...
    const cname=dataa[i].document.companyName;
    const chemicalType = dataa[i].document.psamples;
    const quantity=dataa[i].document.tsamples;
    const submittedOn = formatDateToCustomFormat(dataa[i].createdAt);

    // Create an object with the extracted data
    const entry = [i + 1, names,cname, chemicalType, quantity,submittedOn];
    console.log(entry);
  ...
  doc.autoTable({
    head: [['No.', 'Producer','Company name', 'Chemical Type','Quantity provided', 'Submitted On']], // Use the first row as the table header
  ...
  doc.text('Kigali, Rwanda', applicantTextWidth + 35, 284 + 12 * 3);
...
export default VendorGeneratePDF;
